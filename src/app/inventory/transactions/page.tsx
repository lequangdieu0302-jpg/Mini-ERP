...
export default function TransactionAuditTrail() {
  ...
  const { t } = useERP();
  const { transactions } = useWMSState();
  ...
  const filteredTxns = transactions.filter(tx => {
    const matchesSearch = (tx.product_name && tx.product_name.toLowerCase().includes(searchTerm.toLowerCase())) ||
                          (tx.sku && tx.sku.toLowerCase().includes(searchTerm.toLowerCase())) ||
                          tx.reference_no.toLowerCase().includes(searchTerm.toLowerCase()) ||
                          (tx.performer_name && tx.performer_name.toLowerCase().includes(searchTerm.toLowerCase()));
    
    const matchesAction = selectedAction === 'All' || tx.action === selectedAction.toLowerCase().replace(' ', '_');
    
    // Date filter matching
    let matchesDate = true;
    if (tx.created_at) {
      const txDateStr = tx.created_at.split('T')[0];
      if (fromDate && txDateStr < fromDate) matchesDate = false;
      if (toDate && txDateStr > toDate) matchesDate = false;
    }
              ...
              <tbody>
                {filteredTxns.length === 0 ? (
                  ...
                  filteredTxns.map((tx, idx) => {
                    ...
                    // Date formatting
                    const timeStr = new Date(tx.created_at).toLocaleDateString('vi-VN', {
